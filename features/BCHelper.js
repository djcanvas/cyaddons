import config from "../config";
import RenderLib from "../../RenderLib";
import dungeonRoom from "../events/dungeonRoom";
import Dungeon from "../../BloomCore/dungeons/Dungeon";
import Vector3 from "../../BloomCore/utils/Vector3";
import packetChat from "../events/packetChat";

// Java types
const EntityArmorStand = Java.type("net.minecraft.entity.item.EntityArmorStand");
const S17PacketEntityLookMove = Java.type("net.minecraft.network.play.server.S14PacketEntity$S17PacketEntityLookMove");

// Watcher armor stand skull textures
const skins = ["", "", "", "", "", "", "", "", "", "", "", "", "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvOTZjM2UzMWNmYzY2NzMzMjc1YzQyZmNmYjVkOWE0NDM0MmQ2NDNiNTVjZDE0YzljNzdkMjczYTIzNTIifX19", "", "", "", "", "", "", "", "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNTZmYzg1NGJiODRjZjRiNzY5NzI5Nzk3M2UwMmI3OWJjMTA2OTg0NjBiNTFhNjM5YzYwZTVlNDE3NzM0ZTExIn19fQ==", "", "", ""];


// Module state
let inDungeon = false;
let watcherPhase = 0;
const entities = {};

// Java type import for packet handling
const S32PacketConfirmTransaction = Java.type("net.minecraft.network.play.server.S32PacketConfirmTransaction");

// Blood room watcher chat messages that signal the start of the timer
const BLOOD_START_MESSAGES = [
    "[BOSS] The Watcher: Things feel a little more roomy now, eh?",
    "[BOSS] The Watcher: Oh.. hello?",
    "[BOSS] The Watcher: I'm starting to get tired of seeing you around here...", 
    "[BOSS] The Watcher: You've managed to scratch and claw your way here, eh?", 
    "[BOSS] The Watcher: So you made it this far... interesting.", 
    "[BOSS] The Watcher: Ah, we meet again...", 
    "[BOSS] The Watcher: Ah, you've finally arrived."
];

// Timer variables
let bloodStartTime = Date.now();
let bloodStartTicks = 0;
let bloodMovePredictionTicks = 0;
let displayText = " ";

// Register a handler for when blood room starts
register("chat", (message) => {
    if (!BLOOD_START_MESSAGES.includes(message)) return;
    // Register the helperTrigger initially if config.BCHelper is true
if (config.BCHelper) {
    helperTrigger.register();
}else if (!config.BCHelper){
    helperTrigger.unregister();
}
    bloodStartTime = Date.now();
    bloodStartTicks = 0;
    bloodServerTicks.register();
}).setCriteria("${message}");

// Track server ticks for more accurate timing
const bloodServerTicks = register('packetReceived', () => {
    bloodStartTicks++;
}).setFilteredClass(S32PacketConfirmTransaction).unregister();

// Handle the watcher's "Let's see how you can handle this" message
register("chat", () => {
    if (!config.BCHelper) return;
    
    const bloodMove = ((Math.floor((Date.now() - bloodStartTime)/10)/100) + 0.10).toFixed(2);
    const bloodMoveTicks = (bloodStartTicks*0.05+0.1).toFixed(2);
    const bloodMoveLag = (bloodMove - bloodMoveTicks);

    // Calculate move prediction based on ticks and lag compensation
    if (bloodMoveTicks >= 31 && bloodMoveTicks <= 33.99) {
        bloodMovePredictionTicks = (36 + (bloodMoveLag/2) - 0.6).toFixed(2);
    } else if (bloodMoveTicks >= 28 && bloodMoveTicks <= 30.99) {
        bloodMovePredictionTicks = (33 + (bloodMoveLag/2) - 0.6).toFixed(2);
    } else if (bloodMoveTicks >= 25 && bloodMoveTicks <= 27.99) {
        bloodMovePredictionTicks = (30 + (bloodMoveLag/2) - 0.6).toFixed(2);
    } else if (bloodMoveTicks >= 22 && bloodMoveTicks <= 24.99) {
        bloodMovePredictionTicks = (27 + (bloodMoveLag/2) - 0.6).toFixed(2);
    } else if (bloodMoveTicks >= 1 && bloodMoveTicks <= 21.99) {
        bloodMovePredictionTicks = (24 + (bloodMoveLag/2) - 0.6).toFixed(2);
    }
    
    // Validate prediction
    if (bloodMovePredictionTicks < 20 || bloodMovePredictionTicks > 40) {
        bloodMovePredictionTicks = "Invalid Prediction";
    }
    
    if (config.BCHelper) {
        ChatLib.chat(`&cMove Prediction&b: &3${bloodMovePredictionTicks} Seconds`);
    }
    
    // Set overlay display text and register it
    displayText = `&3${bloodMovePredictionTicks}`;
    bloodOverlay.register();
    
    // Set timeouts for overlay management
    setTimeout(() => {
        bloodOverlay.unregister();
        displayText = `&cKill Mobs`;
    }, 1000);
    
    setTimeout(() => {
        if (config.BCHelper) {
            ChatLib.chat(`&cKill Blood Mobs!`);
        }
        bloodOverlay.register();
    }, parseFloat(((bloodMovePredictionTicks - bloodMoveTicks) * 1000) - 150));
    
    setTimeout(() => {
        bloodOverlay.unregister();
        bloodServerTicks.unregister();
    }, parseFloat(((bloodMovePredictionTicks - bloodMoveTicks) * 1000) + 850));
    
}).setCriteria("[BOSS] The Watcher: Let's see how you can handle this.");

// Overlay rendering
const bloodOverlay = register("renderOverlay", () => {
    if (!config.BCHelper) return;
    
    const scale = 3;
    Renderer.scale(scale);
    Renderer.drawStringWithShadow(
        displayText, 
        (Renderer.screen.getWidth() / scale - Renderer.getStringWidth(displayText)) / 2, 
        Renderer.screen.getHeight() / scale / 2 - 20
    );
}).unregister();




const renderTrigger = register("renderWorld", () => {
	for (let o of Object.entries(entities)) {
		let [k, v] = o;
		RenderLib.drawEspBox(v.final[0], v.final[1] + 1, v.final[2], 0.75, 0.75, 1, 0, 0, 1, true);
		const vec1 = new Vector3(v.final[0], v.final[1], v.final[2]).subtract(new Vector3(v.entity.getRenderX(), v.entity.getRenderY(), v.entity.getRenderZ()));
		const vec2 = new Vector3(v.final[0], v.final[1], v.final[2]).subtract(new Vector3(v.initial[0], v.initial[1], v.initial[2]));
		const progress = vec1.getLength() / vec2.getLength();
		Tessellator.drawString((progress * v.ticks / 20).toFixed(2) + "s", v.final[0], v.final[1] + 1.5, v.final[2]);
		if (v.entity.isDead()) delete entities[k];
	}
}).unregister();

const moveTrigger = register("packetReceived", packet => {
	const mcEntity = packet.func_149065_a(World.getWorld());
	if (!(mcEntity instanceof EntityArmorStand)) return;
	const itemStack = mcEntity.func_82169_q(3);
	if (!itemStack) return;
	const item = new Item(itemStack);
	if (item.getID() !== 397) return;
	const nbtObject = item.getItemNBT()?.toObject();
	if (!nbtObject) return;
	const textures = nbtObject.tag?.SkullOwner?.Properties?.textures?.[0]?.Value;
	if (!textures) return;
	if (!skins.includes(textures)) return;
	const entity = new Entity(mcEntity);
	const [x, y, z] = [entity.getX(), entity.getY(), entity.getZ()];
	if (new Vec3i(x, y, z).distance(new Vec3i(-185 + Math.floor((Player.getX() + 200) / 32) * 32, 75, -185 + Math.floor((Player.getZ() + 200) / 32) * 32)) > 20) return;
	const [dx, dy, dz] = [packet.func_149062_c() / 32, packet.func_149061_d() / 32, packet.func_149064_e() / 32];
	const uuid = entity.getUUID();
	const e = entities[uuid] ??= {
		entity,
		initial: [x, y, z],
		final: [],
		vectors: [],
		ticks: watcherPhase ? 19 : 39
	};
	e.vectors.push(new Vector3(dx, dy, dz));
	const vec = new Vector3(0, 0, 0);
	for (let v of e.vectors) vec.add(v);
	vec.multiply(1 / e.vectors.length * (e.ticks < 20 ? 13.5 : 26.5));
	const finalVec = new Vector3(e.initial[0], e.initial[1], e.initial[2]).add(vec);
	e.final = [finalVec.getX(), finalVec.getY(), finalVec.getZ()];
}).setFilteredClass(S17PacketEntityLookMove).unregister();

function packetChatListener(message) {
	if (["[BOSS] The Watcher: Congratulations, you made it through the Entrance.", "[BOSS] The Watcher: Ah, you've finally arrived.", "[BOSS] The Watcher: Ah, we meet again...", "[BOSS] The Watcher: So you made it this far... interesting.", "[BOSS] The Watcher: You've managed to scratch and claw your way here, eh?", "[BOSS] The Watcher: I'm starting to get tired of seeing you around here...", "[BOSS] The Watcher: Oh.. hello?", "[BOSS] The Watcher: Things feel a little more roomy now, eh?"].includes(message)) {
		watcherPhase = 0;
	} else if (message === "[BOSS] The Watcher: Let's see how you can handle this.") {
		watcherPhase = 1;
	}
}

function dungeonRoomListener(room) {
	if (room === -2130054003) {
		moveTrigger.register();
		renderTrigger.register();
		for (let key in entities) {
			delete entities[key];
		}
	} else {
		moveTrigger.unregister();
		renderTrigger.unregister();
	}
}

// Modified helperTrigger to check config.BCHelper
const helperTrigger = register("step", () => {
    // Only run if config.BCHelper is enabled
    if (config.BCHelper) {
        if (Dungeon.inDungeon) {
            if (inDungeon) return;
            dungeonRoom.addListener(dungeonRoomListener);
            packetChat.addListener(packetChatListener);
            inDungeon = true;
        } else {
            if (!inDungeon) return;
            dungeonRoom.removeListener(dungeonRoomListener);
            packetChat.removeListener(packetChatListener);
            moveTrigger.unregister();
            renderTrigger.unregister();
            inDungeon = false;
        }
    } else if (inDungeon) {
        // If config.BCHelper is disabled but the module is active, disable it
        dungeonRoom.removeListener(dungeonRoomListener);
        packetChat.removeListener(packetChatListener);
        moveTrigger.unregister();
        renderTrigger.unregister();
        inDungeon = false;
    }
}).setFps(1);

// Register a listener for config changes



// Export an empty object instead of the enable/disable functions
export default {};